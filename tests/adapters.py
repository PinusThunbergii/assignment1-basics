...
import cs336_basics.checkpointing as c
...
def run_linear(
    ...
    linear = m.Linear(d_in, d_out)
    linear.load_state_dict({"W" : weights.clone()})
...
def run_embedding(
    ...
    linear = m.Emmbedding(vocab_size, d_model)
    linear.load_state_dict({"E" : weights.clone()})
    # linear.W.data = weights.clone()
    y = linear(token_ids)
    return y
...
def run_swiglu(
    ...
    swiglu.load_state_dict({
        "W1.W": w1_weight.clone(),
        "W2.W": w2_weight.clone(),
        "W3.W": w3_weight.clone()
    })
...
def run_multihead_self_attention(
    ...
    """
    print()
    print(f"{o_proj_weight.shape=}")
    
    multihead_self_attention = m.MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads)
    multihead_self_attention.load_state_dict(
        { 
            "QKV_proj.W" : torch.concat([q_proj_weight.clone(), k_proj_weight.clone(), v_proj_weight.clone()], dim=0),
            "O_proj.W" : o_proj_weight.clone()
        })
    output = multihead_self_attention(in_features)
    return output
...
def run_multihead_self_attention_with_rope(
    ...
    rope = m.RotaryPositionalEmbedding(theta=theta, d_k=d_model//num_heads, max_seq_len=max_seq_len)
    multihead_self_attention = m.MultiHeadSelfAttention(d_model=d_model, num_heads=num_heads)
    multihead_self_attention.load_state_dict(
        { 
            "QKV_proj.W" : torch.concat([q_proj_weight.clone(), k_proj_weight.clone(), v_proj_weight.clone()], dim=0),
    ...
    output = multihead_self_attention(in_features, rope, token_positions)
    return output
...
def run_rope(
    ...
    """
    rope = m.RotaryPositionalEmbedding(theta=theta, d_k=d_k, max_seq_len=max_seq_len)
...
def run_transformer_block(
    ...
    """
    rope = m.RotaryPositionalEmbedding(theta=theta, d_k=d_model // num_heads, max_seq_len=max_seq_len)
    transformer = m.TransformerBlock(d_model, num_heads, d_ff, rope)
...
def run_rmsnorm(
    ...
    rmsnorm = m.RMSNorm(d_model, eps)
    rmsnorm.load_state_dict({"G" : weights.clone()})
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    probs = m.softmax(in_features, dim) 
    return probs
...
def run_save_checkpoint(
    ...
    """
    c.save_checkpoint(model, optimizer, iteration, out)
...
def run_load_checkpoint(
    ...
    """
    return c.load_checkpoint(src, model, optimizer)
...
def get_tokenizer(
    ...
    """
    # raise NotImplementedError
    import cs336_basics.tokenizer as t
    return t.Tokenizer(vocab, merges, special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    import cs336_basics.bpe as b
    return b.train_bpe(input_path, vocab_size, special_tokens)
